...
export class GameLogUtils {
  ...
  static getVenueIndicator(gameLog: GameLog): "vs" | "@" {
    return gameLog.isHome === 1 ? "vs" : "@";
  }

  /*Calculate player efficiency rating for the game*/
  static calculateGamePER(gameLog: GameLog): number {
    const minutes = this.timePlayedToMinutes(gameLog.timePlayed);
    ...
    const per =
      (gameLog.pts +
        ...
        gameLog.stl +
        gameLog.blk -
        (gameLog.fga - gameLog.fgm) -
        (gameLog.fta - gameLog.ftm) -
        gameLog.tov) /
      minutes;
  ...
  /*Check if performance was a double-double*/
  static isDoubleDouble(gameLog: GameLog): boolean {
    const stats = [
      gameLog.pts,
      gameLog.reb,
      gameLog.ast,
  ...
  /*Check if performance was a triple-double*/
  static isTripleDouble(gameLog: GameLog): boolean {
    const stats = [
      gameLog.pts,
      gameLog.reb,
      gameLog.ast,
